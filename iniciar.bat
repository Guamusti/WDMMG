...
echo.
set "API_REUSED="
for /f %%A in ('powershell -NoProfile -Command "$root=(Get-Location).Path; Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and $_.CommandLine -like ('*' + $root + '*api/server.mjs*') } | ForEach-Object { if ($_.CommandLine -match 'ATLAS_API_PORT[= ](\d+)') { $matches[1] } } | Select-Object -First 1"') do set "APIPORT=%%A"
if not defined APIPORT for /f %%A in ('powershell -NoProfile -Command "$f='public/api-port.json'; if (Test-Path $f) { try { $p=(Get-Content -Raw $f | ConvertFrom-Json).port; $r=Invoke-WebRequest -UseBasicParsing -Uri ('http://127.0.0.1:'+ $p +'/api/health') -TimeoutSec 1; if ($r.StatusCode -eq 200) { $p } } catch {} }"') do set "APIPORT=%%A"
