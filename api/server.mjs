...
function qualityReport() {
  ...
  return [
    { id: 'placsp', name: 'Contratos PLACSP', records: contracts.length, missingIds: contracts.length - contractIds.length, duplicates: duplicateCount(contractIds), anomalies: contracts.filter(row => !row.source_url || !validDate(row.publication_date) || !validAmount(row.estimated_value)).length, sourceUrl: 'https://contrataciondelestado.es/' },
    { id: 'placsp-awards', name: 'Adjudicaciones PLACSP', records: awards.length, missingIds: awards.filter(row => !row.award_id).length, duplicates: duplicateCount(awards.map(row => row.award_id).filter(Boolean)), anomalies: awards.filter(row => (row.award_amount == null && row.winner_name) || !validDate(row.award_date) || !validAmount(row.award_amount) || !validAmount(row.award_amount_with_tax)).length, sourceUrl: 'https://contrataciondelestado.es/' },
