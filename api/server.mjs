...
async function databaseCoverage() {
  ...
  return [
    ...databaseRows,
    {
      ...
      last_checked_at: ccaaRows[0]?.retrieved_at || null,
      last_imported_at: ccaaRows[0]?.retrieved_at || null,
...
function qualityReport() {
  ...
  const summarizeFlags = rows => { const counts = {}; const examples = {}; for (const row of rows) for (const flag of row.quality_flags || []) { counts[flag] = (counts[flag] || 0) + 1; if (!examples[flag]) examples[flag] = row.source_record_id || row.award_id || null; } return { flagCounts: counts, flagExamples: examples }; };
  const validDate = value => !value || !Number.isNaN(Date.parse(String(value)));
  const validAmount = value => value == null || (String(value).trim() !== '' && Number.isFinite(Number(value)) && Number(value) >= 0);
  return [
    { id: 'placsp', name: 'Contratos PLACSP', records: contracts.length, missingIds: contracts.length - contractIds.length, duplicates: duplicateCount(contractIds), anomalies: contracts.filter(row => !row.source_url || !validDate(row.publication_date) || !validAmount(row.estimated_value)).length, ...summarizeFlags(contracts), sourceUrl: 'https://contrataciondelestado.es/' },
    { id: 'placsp-awards', name: 'Adjudicaciones PLACSP', records: awards.length, missingIds: awards.filter(row => !row.award_id).length, duplicates: duplicateCount(awards.map(row => row.award_id).filter(Boolean)), anomalies: awards.filter(row => (row.award_amount == null && row.winner_name) || !validDate(row.award_date) || !validAmount(row.award_amount) || !validAmount(row.award_amount_with_tax)).length, ...summarizeFlags(awards), sourceUrl: 'https://contrataciondelestado.es/' },
    { id: 'igae', name: 'Ejecución AGE · mayo 2026', records: execution.length, missingIds: execution.filter(row => !row.source_record_id).length, duplicates: duplicateCount(execution.map(row => row.source_record_id).filter(Boolean)), anomalies: execution.filter(row => (row.quality_flags || []).length || !validAmount(row.final_credit) || !validAmount(row.paid_amount)).length, missingFields: [...new Set(['initial_credit', 'budget_modifications'].filter(field => execution.length && execution.every(row => row[field] == null)))], sourceUrl: execution[0]?.source_url || null },
    { id: 'igae-policies', name: 'Partidas funcionales IGAE · 2024', records: policies.length, missingIds: policies.filter(row => !row.code).length, duplicates: duplicateCount(policies.map(row => row.code).filter(Boolean)), anomalies: policies.filter(row => !validAmount(row.amount)).length, sourceUrl: 'https://www.igae.pap.hacienda.gob.es/' },
...
async function databaseMetrics(period = '') {
  const cacheKey = period || 'latest';
  const cached = metricsCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.data;
  const [execution, contracts, grants] = await Promise.all([
    pool.query(`SELECT SUM(br.final_amount) AS final_credit, SUM(be.recognized_amount) AS recognized, SUM(be.paid_amount) AS paid, MAX(br.period) AS period FROM budget_records br JOIN budget_execution be ON be.budget_record_id = br.id WHERE br.economic_level = 'chapter' AND br.economic_code ~ '^[1-9]\\. ' AND ($1 = '' OR br.period = $1)`, [period]),
  ...
  const fallback = execution.rows[0].period ? null : overviewFromJsonl(period);
  const executionData = fallback?.execution || execution.rows[0];
  const data = {
    budget: { final_credit: executionData.final_credit ?? executionData.finalCredit, period: fallback?.period || executionData.period, unit: 'miles de euros', source: 'IGAE' },
    execution: { recognized: executionData.recognized ?? executionData.recognized_amount, paid: executionData.paid ?? executionData.paid_amount, period: fallback?.period || executionData.period, unit: 'miles de euros', source: 'IGAE' },
  ...
  metricsCache.set(cacheKey, { data, expiresAt: Date.now() + 30000 });
  return data;
}
  ...
  if (url.pathname === '/api/export.csv') {
    ...
    try {
      ...
      if (entity === 'budgets') {
        const search = `%${query}%`;
        try {
          const result = await pool.query(`SELECT br.fiscal_year, br.period, br.economic_code, br.economic_level, br.initial_amount, br.modifications_amount, br.final_amount, be.committed_amount, be.recognized_amount, be.paid_amount, br.final_amount - COALESCE(be.recognized_amount, 0) AS remaining_credit, CASE WHEN br.final_amount > 0 THEN (be.recognized_amount / br.final_amount) * 100 ELSE NULL END AS execution_percentage, CASE WHEN br.final_amount > 0 THEN (be.paid_amount / br.final_amount) * 100 ELSE NULL END AS payment_percentage, ds.source_url FROM budget_records br LEFT JOIN budget_execution be ON be.budget_record_id = br.id JOIN data_sources ds ON ds.id = br.source_id WHERE ($1 = '' OR br.economic_code ILIKE $2) ORDER BY br.fiscal_year DESC, br.period DESC, br.id`, [query, search]);
          return csv(res, 'presupuesto-igae.csv', result.rows.map(budgetExecutionFields));
        } catch (error) {
          const normalized = query.toLocaleLowerCase('es');
          const rows = getExecution().filter(row => !query || [row.economic_code, row.classification_code, row.classification_label, row.economic_label].some(value => String(value || '').toLocaleLowerCase('es').includes(normalized))).map(budgetExecutionFields);
          return csv(res, 'presupuesto-igae.csv', rows);
        }
      }
