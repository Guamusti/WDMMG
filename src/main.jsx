  ...
  return <div className="app">
    ...
    <main>
      ...
      {view === 'methodology' ? <Methodology /> : view === 'contracts' ? <Contracts rows={contracts} loading={contractsLoading} query={query} /> : view === 'grants' ? <Grants /> : <>
        <section className="policy-section"><div className="section-head policy-head"><div><span className="eyebrow">EL DESGLOSE QUE SE ENTIENDE · CUENTA GENERAL DEL ESTADO 2024</span><h2>¿En qué se gastó el dinero?</h2><p className="section-subtitle">La rueda muestra el peso de cada finalidad. Es el gasto realizado de todo 2024; no se mezcla con la ejecución mensual de 2026 que aparece más abajo.</p></div><a className="source-mini" href="https://www.igae.pap.hacienda.gob.es/sitios/igae/es-ES/Contabilidad/ContabilidadPublica/CPE/EjecucionPresupuestaria/Documents/C.G.E.%202024.pdf" target="_blank">Fuente IGAE ↗</a></div><PolicyWheel selected={selectedPolicy} onSelect={setSelectedPolicy} />{selectedPolicy && <div className="policy-drilldown" aria-live="polite"><div><span className="eyebrow">HAS ELEGIDO</span><h3>{selectedPolicy.label}</h3><strong className="policy-share">{percent(selectedPolicy.amount, functionalTotal)}%</strong><p>del gasto público total · {moneyMillions(selectedPolicy.amount)} M€ en 2024.</p></div>{selectedPolicy.children ? <div className="policy-children">{selectedPolicy.children.map(child => <div key={child.label}><b>{child.label}</b><strong>{moneyMillions(child.amount)} M€</strong><small>{percent(child.amount, selectedPolicy.amount)}% de esta partida</small></div>)}</div> : <div className="drilldown-empty"><strong>La fuente no baja más al detalle en esta tabla.</strong><span>Cuando exista un desglose oficial —por ejemplo, una separación verificable de prestaciones— aparecerá aquí; no rellenamos el hueco con estimaciones.</span></div>}</div>}</section>
