...
function Territories({ rows, loading }) { const communities = rows.filter(row => row.territory !== 'Total CCAA'); return <section className="territories"><div className="section-head compact"><div><span className="eyebrow">04 · OTRA ESCALA</span><h2>¿Qué pasa en las comunidades?</h2><p className="section-subtitle">Ejecución acumulada hasta mayo de 2026. Son datos autonómicos y no se suman a la AGE.</p></div><span className="territory-status">{loading ? 'Cargando…' : `${communities.length} comunidades`}</span></div>{loading ? <div className="empty-row">Consultando CIMCANET…</div> : <div className="territory-grid">{communities.map(row => <div className="territory-card" key={row.territory}><b>{row.territory}</b><strong>{millions(row.recognized_expense_non_financial)} M€</strong><small>gasto no financiero reconocido · avance</small></div>)}</div>}</section> }
