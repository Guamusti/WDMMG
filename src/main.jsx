...
function App() {
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <nav aria-label="Secciones principales"><button aria-current={view === 'overview' ? 'page' : undefined} className={view === 'overview' ? 'active' : ''} onClick={() => setView('overview')}>Explorar</button><button aria-current={view === 'contracts' ? 'page' : undefined} className={view === 'contracts' ? 'active' : ''} onClick={() => setView('contracts')}>Contratos</button><button aria-current={view === 'grants' ? 'page' : undefined} className={view === 'grants' ? 'active' : ''} onClick={() => setView('grants')}>Subvenciones</button><button aria-current={view === 'methodology' ? 'page' : undefined} className={view === 'methodology' ? 'active' : ''} onClick={() => setView('methodology')}>Metodología</button></nav>
    ...
    <main>
      ...
      {view === 'methodology' ? <Methodology /> : view === 'contracts' ? <Contracts rows={contracts} loading={contractsLoading} query={query} /> : view === 'grants' ? <Grants /> : <>
        ...
        <Territories rows={territories} loading={territoriesLoading} />
...
function coverageStatus(source) { if (source.data_status === 'partial') return ['Parcial', 'partial']; if (source.data_status === 'blocked_reader') return ['Pendiente: lector Access', 'blocked']; return [Number(source.budget_records) + Number(source.contract_records) + Number(source.grant_records) ? 'Cargado' : 'Localizado']; }
function Methodology() { const [coverage, setCoverage] = useState([]); useEffect(() => { fetch('http://localhost:8787/api/coverage').then(response => response.ok ? response.json() : { data: [] }).then(payload => setCoverage(payload.data || [])).catch(() => setCoverage([])); }, []); return <section className="methodology"><span className="eyebrow">METODOLOGÍA Y COBERTURA</span><h2>Una cifra, una definición, una fuente.</h2><p>El producto separa presupuesto, ejecución, contratos y subvenciones. No suma magnitudes que puedan solaparse y no infiere relaciones entre una partida y una adjudicación sin evidencia.</p><div className="glossary"><span className="eyebrow">PARA LEERLO SIN TRADUCCIÓN</span><h3>¿Qué significa cada cifra?</h3><div className="glossary-grid"><div><b>Presupuesto</b><span>Lo que estaba previsto gastar.</span></div><div><b>Gasto registrado</b><span>Lo que el Estado ya reconoce como obligación.</span></div><div><b>Dinero pagado</b><span>Lo que ya ha salido efectivamente.</span></div><div><b>Contratos</b><span>Compras y obras publicadas aparte.</span></div></div></div><div className="coverage-grid">{coverage.map(source => { const [label, statusClass] = coverageStatus(source); return <div className="coverage-card" key={source.id}><div className={`coverage-status ${statusClass || ''}`}><span aria-hidden="true" />{label}</div><span className="eyebrow">{source.institution}</span><h3>{source.name}</h3><p>{source.coverage_description || 'Cobertura documentada en preparación.'}</p><div><b>{Number(source.budget_records) + Number(source.contract_records) + Number(source.grant_records)}</b><small> registros cargados</small></div><a href={source.source_url} target="_blank">Fuente original <ExternalLink size={13}/></a></div> })}</div><div className="source-list">{[['Hacienda / IGAE','Ejecución AGE mensual y presupuestos',sources.igae],['PLACSP','Feeds abiertos ATOM/XML de licitaciones',sources.placsp],['BDNS','Servicios web y especificaciones BDNS20',sources.bdns],['INE','API JSON y población municipal oficial',sources.ine]].map(([n,d,u]) => <a href={u} target="_blank" className="source-item" key={n}><span><b>{n}</b><small>{d}</small></span><ExternalLink size={16}/></a>)}</div></section> }
