...
function PolicyWheel({ selected, onSelect }) { let cursor = -Math.PI / 2; return <div className="policy-wheel-layout"><div className="policy-wheel-wrap"><svg className="policy-wheel" viewBox="0 0 360 360" role="img" aria-labelledby="policy-wheel-title policy-wheel-description"><title id="policy-wheel-title">Reparto del gasto público por finalidad</title><desc id="policy-wheel-description">Pulsa un sector para ver su porcentaje y el desglose disponible.</desc>{policyWheelItems.map(item => { const start = cursor; const end = cursor + (item.amount / functionalTotal) * Math.PI * 2; cursor = end; return <path key={item.id} className={selected?.id === item.id ? 'active' : ''} d={donutPath(180, 180, 150, 78, start, end)} fill={item.color} tabIndex="0" role="button" aria-label={`${item.label}: ${percent(item.amount, functionalTotal)} por ciento`} aria-pressed={selected?.id === item.id} onClick={() => onSelect(item)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelect(item); } }}><title>{`${item.label}: ${percent(item.amount, functionalTotal)}%`}</title></path> })}<text x="180" y="166" className="wheel-center-label">{selected ? selected.label : 'ELIGE UNA PARTIDA'}</text><text x="180" y="204" className="wheel-center-value">{selected ? `${percent(selected.amount, functionalTotal)}%` : '2024'}</text></svg></div><div className="policy-legend" aria-label="Partidas de gasto">{policyWheelItems.map(item => <button type="button" key={item.id} className={selected?.id === item.id ? 'selected' : ''} onClick={() => onSelect(item)}><i style={{ background: item.color }} /><span>{item.label}</span><strong>{percent(item.amount, functionalTotal)}%</strong></button>)}</div></div> }
