    ...
    with psycopg.connect(database_url) as connection:
        with connection.cursor() as cursor:
            ...
            for row in rows:
                ...
                cursor.execute("""
                    INSERT INTO budget_records (fiscal_year, period, entity_id, budget_side, economic_code, economic_level, final_amount, data_status, source_id, source_record_id, ingestion_run_id)
                    VALUES (%s,%s,%s,'expense',%s,%s,%s,%s,%s,%s,%s) RETURNING id
                """, (row["fiscal_year"], row["period"], entity_id, row["classification_label"], row["classification_level"], row["final_credit"], row["data_status"], source_id, row["source_record_id"], ingestion_id))
